...
def initial_background(img, sample_num):
    ...
    width = img.shape[1]
    # 创建样本集，彩色图像
    samples = np.zeros((img.shape[0], img.shape[1], sample_num, 3))
...
def classify(img, samples, sample_num, radius, min_matches):
    ...
    width = img.shape[1]
    # 用于保存欧式距离
    distance = np.zeros((height, width, sample_num, 3))
    # 计算欧式距离x1-x2
    ...
    # 计算匹配个数
    count = np.where(distance < radius * radius, 1, 0)
